...
public class Interface {
	...
	public Interface(int x, int y) {
		...
		try {
			localSave = Interface.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath(); // salva onde java esta rodando
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	private void spawnaAleatorio() {
		...
		while(invalido) {
			...
			if(atual == null) {
				int valor = random.nextInt(10) < 9 ? 2 : 4;
	...
	public void processa(Graphics2D graficos) {
		...
		graficos.drawString("Recorde: " + recorde, Jogo.LARGURA - DesenhosAuxiliares.recebeLarguraMensagem("Recorde: " + recorde, 
				fontePontuacao, graficos) - 20, 40);
	...
	private boolean move(int linha, int coluna, int direcaoHorizontal, int direcaoVertical, Direcao direcao) {
		...
		Bloco atual = interfaceJogo[linha][coluna];
		if(atual == null) return false; // nao da pra mover posicao vazia
		boolean mover = true;
		int novaLinha = linha;
		...
		while(mover) { //enquanto puderem combinar ou deslizar para um bloco vazio
			...
			if(verificaLimites(direcao, novaLinha, novaColuna)) break;
			if(interfaceJogo[novaLinha][novaColuna] == null) { //posso mover
				interfaceJogo[novaLinha][novaColuna] = atual; // move para vazio
				interfaceJogo[novaLinha - direcaoVertical][novaColuna - direcaoHorizontal] = null; //esvazia posicao anterior
				interfaceJogo[novaLinha][novaColuna].setDeslizaPara(new Ponto(novaLinha, novaColuna));
				podeMover = true;
			}
			else if(interfaceJogo[novaLinha][novaColuna].buscaValor() == atual.buscaValor() && interfaceJogo[novaLinha][novaColuna].podeUnir()) {
				interfaceJogo[novaLinha][novaColuna].setPodeUnir(false);
				interfaceJogo[novaLinha][novaColuna].setValor(interfaceJogo[novaLinha][novaColuna].buscaValor() * 2); //multiplica valor por 2
				podeMover = true;
		...
		if(direcao == Direcao.ESQUERDA) {
			...
			for(int linha = 0; linha < LINHAS; linha++) {
				for(int coluna = 0; coluna < COLUNAS; coluna++) {
					...
					else move(linha, coluna, direcaoHorizontal, direcaoVertical, direcao);
				}
			}
		}
		else if(direcao == Direcao.DIREITA) {
	...
	private void verificaDerrota() {
		for(int linha = 0; linha < LINHAS; linha++) {
			for(int coluna = 0; coluna < COLUNAS; coluna++) {
				if(interfaceJogo[linha][coluna] == null) return; //existe posicao vazia, portanto nao perdeu
	...
	private boolean verificaBlocosEmVolta(int linha, int coluna, Bloco atual) {
		if(linha > 0) {
			...
			if(atual.buscaValor() == checar.buscaValor()) return true;
		}
		if(linha < LINHAS - 1) {
			Bloco checar = interfaceJogo[linha + 1][coluna];
			if(checar == null) return true;
