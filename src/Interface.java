...
public class Interface extends Inicio {
	...
	private Bloco[][] posicao; //define matriz do jogo
	...
	private void inicia() {
		i.perguntaXastre(); // pergunta ao usuario
		for (int i = 0; i < numBlocosInicio; i++) {
			spawnaAleatorio();
		}
	}

	private void spawnaAleatorio() { //spawna n numeros aleatorios no inicio do jogo (definido como constante 2)
		...
		while (invalido) {
			...
			if (posicaoX == null) {
				...
				else if(i.opcao == JOptionPane.YES_OPTION){
	...
	private void moveBlocos(Direcao direcao) {
		...
		for (int linha = 0; linha < LINHAS; linha++) {
			for (int coluna = 0; coluna < COLUNAS; coluna++) {
				...
				if (posicaoX == null)
					continue;
				posicaoX.setPodeUnir(true); //se a posicao nao esiver vazia, é uma peca que pode ser unida a outra peca
			}
		}
